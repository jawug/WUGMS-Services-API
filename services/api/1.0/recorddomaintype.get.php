...
if ($page_data->PageActions->getStatus()) {
    ...
    if (array_key_exists('parameters', $wsapi_data)) {
        try {
            $domain_id = $EntryValidation->validateCheckNumeric($wsapi_data['parameters'][1]);
        } catch (Exception $e) {
...
if ($page_data->PageActions->getStatus()) {
    ...
    if (array_key_exists('parameters', $wsapi_data)) {
        try {
            $domain_type = $EntryValidation->validateStringRegex($wsapi_data['parameters'][2], $EntryValidation->expressions->getRegexDomainType());
        } catch (Exception $e) {
            $page_data->PageActions->setStatus(FALSE);
            $page_data->PageActions->setStatusCode($e->getMessage());
            $page_data->PageActions->setExtendedStatusCode("Sub function located in " . $e->getFile());
...
if ($page_data->PageActions->getStatus()) {
    /* SQL - Query */
    $get_record_domain_type_details_query = "SELECT a.id, a.domain_id, a.name, a.type, a.content, a.ttl, a.prio, a.change_date, a.disabled, a.ordername, a.auth FROM powerdns.records a WHERE a.domain_id = :domain_id AND a.type = :type_id;";
    /* SQL - Params */
    $get_record_domain_type_details_query_params = array(
        ':domain_id' => $domain_id,
        ':type_id' => $domain_type
    );
    /* SQL - Exec */
    try {
        $get_record_domain_type_details_stmt = $page_data->DAO_Service->DAO_Service->prepare($get_record_domain_type_details_query);
        $get_record_domain_type_details_result = $get_record_domain_type_details_stmt->execute($get_record_domain_type_details_query_params);
    } catch (PDOException $ex) {
        /* SQL - Error Handling */
        $page_data->PageActions->setStatus(FALSE);
        $page_data->PageActions->setStatusCode("get_record_domain_type_details_stmt");
    ...
    if ($page_data->PageActions->getStatus()) {
        $get_record_domain_type_details_data_row = $get_record_domain_type_details_stmt->fetchAll();
        if ($get_record_domain_type_details_data_row) {
            $page_data->PageData->PageWebStatus->setAPIResponse(1);
            $page_data->PageData->PageWebStatus->setAPIResponseData($get_record_domain_type_details_data_row);
            $page_data->PageActions->setStatusCode("Data Size: " . count($get_record_domain_type_details_data_row) . " row(s)");
        } else {
            /* There was no data */
