...
if ($page_data->PageActions->getStatus()) {
    /* SQL - Query */
    $get_users_list_query = "SELECT a.id, a.username, a.attribute, a.op, a.value FROM radius.radcheck a;";
    /* SQL - Exec */
    try {
        $get_users_list_stmt = $page_data->DAO_Service->DAO_Service->prepare($get_users_list_query);
        $get_users_list_result = $get_users_list_stmt->execute();
    } catch (PDOException $ex) {
        /* SQL - Error Handling */
        $page_data->PageActions->setStatus(FALSE);
        $page_data->PageActions->setStatusCode("get_users_list_stmt");
    ...
    if ($page_data->PageActions->getStatus()) {
        $get_users_list_data_row = $get_users_list_stmt->fetchAll();
        if ($get_users_list_data_row) {
            $page_data->PageData->PageWebStatus->setAPIResponse(1);
            $page_data->PageData->PageWebStatus->setAPIResponseData($get_users_list_data_row);
            $page_data->PageActions->setStatusCode("Data Size: " . count($get_users_list_data_row) . " row(s)");
        } else {
            /* There was no data */
